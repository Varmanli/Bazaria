...
function ProductUI() {
  ...
  return (
    <div className="flex mt-[70px] flex-col justify-between items-center m-6 text-[#78254E]">
      <div className="lg:flex  lg:justify-between lg:mb-[30px]  ">
        <div className="flex justify-center">
          <img src={product?.image} className="w-[300px] h-[300px]" />
        </div>
        <div className="flex flex-col">
          <div className=" my-5 pb-2 w-[100%] text-black">
            <h1 className="text-[20px] font-bold lg:text-[25px]">
              {product?.title}
            </h1>
          </div>
          <div className="lg:flex  lg:flex-row-reverse justify-around">
            <div className=" flex flex-col-reverse mt-5 items-center gap-6  border-b border-black-100 pb-6 justify-center  border bg-[#F7F7F8] rounded-lg p-5 lg:mr-5 ">
              <div className="flex items-center gap-2 ">
                <AiOutlineSafetyCertificate />
                <h1>گارانتی ۱۸ ماهه آرمان همراه ارتباطات آریا</h1>
              </div>

              <div className="flex sticky bottom-0 gap-10 lg:flex-col-reverse lg:gap-2">
                <Shopping
                  id={product?.id}
                  title={product?.title}
                  price={product?.price}
                  image={product?.image}
                />
                <div className="flex font-bold gap-3 text-[22px] justify-center items-center text-left lg:text-[27px]">
                  <h1>{numberWithCommas(product?.price)}</h1>
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 14 16"
            ...
            <div>
              <div className="flex rounded-lg h-[90px] mt-6 p-4 shadow-xl items-center  gap-2 ">
                <h1 className="text-[20px] w-[290px]">
                  ارسال رایگان برای این کالا
                </h1>
                <img src="https://www.digikala.com/statics/img/svg/digiplus/free-delivery.svg" />
              </div>
              <div className="flex text-[40px] text-gray-600 gap-5 mt-5 justify-center items-center">
                <BiSolidLike className="cursor-pointer" />
                <BiSolidDislike className="cursor-pointer" />
                <IoIosNotifications />
                <FaShareAlt className="text-[30px] cursor-pointer" />
                <FaCommentAlt className="text-[30px] cursor-pointer" />
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className=" items-baseline w-full my-5 flex justify-around ">
        <div className="flex flex-col text-[10px] lg:flex-row items-center lg:text-[18px]">
          <img src={daysReturn} className="h-[60px]" />
          <h1>هفت روز ضمانت بازگشت کالا</h1>
        </div>
        <div className="flex flex-col text-[10px] lg:flex-row items-center  lg:text-[18px]">
          <img src={express} className="h-[60px]" />
          <h1>تحویل اکسپرس</h1>
        </div>
        <div className="flex flex-col text-[10px] lg:flex-row items-center lg:text-[18px]">
          <img src={support} className="h-[60px]" />
          <h1>7 روز هفته 24 ساعت</h1>
        </div>
        <div className="flex flex-col text-[10px] lg:flex-row items-center lg:text-[18px]">
          <img src={original} className="h-[60px]" />
          <h1>ضمانت اصالت کالا</h1>
        </div>
        <div className="flex flex-col text-[10px] lg:flex-row items-center lg:text-[18px]">
          <img src={cash} className="h-[60px]" />
          <h1>پرداخت در محل</h1>
        </div>
      </div>
      <div className="my-5 w-[100%] ">
        <div className="flex gap-4 pb-2   ">
          <button
            className={`${open1 ? "border-b-2  border-red-700 " : ""} text-lg `}
            onClick={() => {
              setOpen1(!open1);
              setOpen3(false);
            }}
          ...
          <button
            className={`${open3 ? "border-b-2  border-red-700 " : ""} text-lg `}
            onClick={() => {
              setOpen3(!open3);
              setOpen1(false);
            }}
        ...
        <div
          className={`${
            open1 ? "block" : "hidden"
          } shadow-lg rounded-lg   my-4 p-4`}
        >
          <h1 className="text-[20px] font-semibold mb-2">معرفی</h1>
          <p>{product?.descreption}</p>
        </div>
        <div className={`${open3 ? "block" : "hidden"} `}>
          <div className="border-b  border-red-600 my-7 pb-6">
